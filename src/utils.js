...
export const randStringArray = (n, stringLength = 5) => {
    return [...Array(n)].map(() => makeid(stringLength));
}

// https://gist.github.com/0x263b/2bdd90886c2036a1ad5bcf06d6e6fb37
export const stringToColor = function (string) {
...
export const objectsToTable = (stats) => {
...
export const swap = (items, i, j) => {
    const temp = items[i];
    items[i] = items[j];
    items[j] = temp;
}

export const tweenPromise = (tween) => {
    return new Promise((resolve) => tween.call(resolve));
}

export const randIntArray = (n, min, max) => {
    const nums = [];
    for (let i = 0; i < n; i++) {
